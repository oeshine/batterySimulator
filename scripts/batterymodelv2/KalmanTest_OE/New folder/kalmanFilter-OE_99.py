...
data2=np.loadtxt('OCV_cell0_Realdata.txt',dtype=np.float)#dataList.append((device.time,device.current,device.voltage,device.soc))
realSocTimeArr=data2[:,0]
realSocArr=data2[:,3]

#parameter of the battery model
data3=np.loadtxt('Book1.txt',dtype=np.float)
ministVoltage=[]
voltage=data3[:,2:-1:4]
for i in range(len(voltage[:,1])):
    ministVoltage.append(min(voltage[i]))
realVoltageArr=np.array(ministVoltage)
