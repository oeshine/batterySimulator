...
X=np.array([[0.5],[0]])#2592.14096604,3.6472383191 soc :0.4890832011396226
P = np.diag((0.01, 0.01))
A=np.array([[1,0],[0,np.exp(-1/(Rp*Cp))]])
Q = np.eye(X.shape[0]) 
B = np.array([[1/Qn],[Rp*(1-np.exp(-1/(Rp*Cp)))]])
U = np.array([[0]])
# print 'X',X
# print 'P',P
# print 'A',A
...
#ax.plot(realSocTimeArr[:N_iter*2],realSocArr[:N_iter*2],'r--',label='Real SOC')
ax.grid(True)
...
#ax2.plot(realSocTimeArr[:N_iter*2],realVoltageArr[:N_iter*2],'b--',label='OCV voltage')
...
ax4.plot(range(0, N_iter),kalmanGainArr[:,0,0],'r',label='Kalman Gain SOC')
ax4.plot(range(0, N_iter),kalmanGainArr[:,1,0],'g',label='Kalman Gain VRC')
ax4.grid(True)
