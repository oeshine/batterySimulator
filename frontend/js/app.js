var NumberOfBatteries = 96
var NumberOfCells = 12
  ...
  if (1) {
    if (typeof gcode === "string" && gcode != '') {
      $.ajax({
        ...
        error: function (data) {
          $().uxmessage('error', "Timeout. BSM App server down?");
        },
...
function save_and_add_to_job_queue(name, gcode) {  
  ...
  //// store gcode - on success add to queue
	$.post("/queue/save", { 'command_list_name':name, 'command_program':gcode }, function(data) {
		if (data == "1") {
		  queue_num_index += 1;
      ...
      save_and_add_to_job_queue(name+' - '+ queue_num_index, gcode);
			...
			$().uxmessage('error', "Failed to store python code.");
		}
  });
}
...
function add_to_job_queue(name) {
  ...
  $.each($('#command_list_queue li'), function(index, li_item) {
  ...
  if (name.slice(-8) == '.starred') {
	...
	$('#command_list_queue').prepend('<li><a href="#"><span>'+ name +'</span><span class="starwidget '+ star_class +' pull-right" title=" star to keep in queue"></span></a></li>')
	$('span.starwidget').tooltip({delay:{ show: 1500, hide: 100}})  
  //// action for loading gcode
	$('#command_list_queue li:first a').click(function(){
  ...
  //// action for star
  $('#command_list_queue li:first a span.starwidget').click(function() {
...
function add_to_library_queue(gcode, name) {
  if ((typeof(name) == 'undefined') || ($.trim(name) == '')) {
	...
	$('#command_list_library').prepend('<li><a href="#"><span>'+ name +'</span><i class="icon-star pull-right"></i><div style="display:none">'+ gcode +'</div></a></li>')
	
	$('#command_list_library li a').click(function(){
	  load_into_command_widget($(this).next().text(), $(this).text())
	});

	$('#command_list_library li a i').click(function(){
...
$(document).ready(function(){
  ...
  $().uxmessage('notice', "BSM App frontend started");  
  ...
  function poll_hardware_status() {
    $.getJSON('/status', function(data) {
      //alert(data.TotalVoltage)
	  try
	  {
        updateIvChart(data.totalCurrent,data.totalVoltage)
  ...
  $("#reset_simulator").click(function(e){
    $.get('/reset', function(data) {
      if (data != "") {
        $().uxmessage('success', "Simulator restarted!");
      } else {
        $().uxmessage('error', "Simulator restart failed!");
      }   
    });
    e.preventDefault();   
  });
